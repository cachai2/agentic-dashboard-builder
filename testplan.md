...
| **GW-03** | Gateway `/json` happy path | Validate proxy sends request to ACA and returns JSON to caller | **Terminal A:** keep gateway from GW-01 running. **Terminal B:** run `python app/agent-service/ping_gateway.py`; inspect response and gateway logs | `/json` returns parsed object; gateway logs show upstream latency + `provider_response.model=llama3.1:8b` |
